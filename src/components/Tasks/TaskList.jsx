...
const TaskList = ({ tasks, setTasks }) => {
  ...
  const getCardStyle = (task) => {
    if (isOverdue(task)) {
      return {
        boxShadow: '0 0 25px rgba(255, 50, 50, 0.5)',
    ...
    switch (task.status) {
      case 'complete':
        return { boxShadow: '0 0 15px rgba(61, 214, 140, 0.3)' };
      case 'in-progress':
        return { boxShadow: '0 0 15px rgba(255, 200, 100, 0.3)' };
      default:
        return { boxShadow: '0 0 15px rgba(255, 100, 100, 0.3)' };
    }
  };
        ...
        {tasks.map((task) => {
          ...
          return (
            <motion.div
              ...
              transition={{
                duration: 0.3,
                opacity: isJustCompleted ? { delay: 1.2, duration: 0.3 } : { duration: 0.3 },
              ...
              className={`relative bg-bg-secondary rounded-xl p-4 border transition-all cursor-move ${
            ...
            >
              ...
              <div className="flex items-start gap-4">
                {/* Drag Handle */}
                <div className="mt-1 text-text-tertiary hover:text-green-glow transition-colors cursor-grab active:cursor-grabbing">
                  ...
                  <div className="flex items-center gap-4 text-xs text-text-tertiary flex-wrap">
                    {task.dueDate && (
                      <span className={`flex items-center gap-1 ${taskIsOverdue ? 'text-red-500 font-bold' : ''}`}>
                        {taskIsOverdue && <AlertCircle size={12} />}
                        <Clock size={12} />
